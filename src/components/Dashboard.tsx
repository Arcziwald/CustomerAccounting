...
// Przypomnienia podatkowe (odwzorowane z produkcji brakomatpro — cron tax-reminders)
type TypPodatkowy = 'ZUS' | 'VAT' | 'PIT' | 'CIT' | 'JPK' | 'custom';
interface Przypomnienie { typ: TypPodatkowy; label?: string; dzien: number; active: boolean }
const TYP_PODATKOWE: Record<TypPodatkowy, string> = {
  ZUS: 'ZUS — składki ubezpieczeniowe',
  VAT: 'VAT-7 / JPK_VAT — deklaracja',
  PIT: 'PIT — zaliczka na podatek',
  CIT: 'CIT — zaliczka na podatek',
  JPK: 'JPK_VAT — ewidencja',
  custom: 'Własny termin',
};
const DEFAULT_PRZYPOMNIENIA: Przypomnienie[] = [
  { typ: 'ZUS', dzien: 15, active: true },
  { typ: 'VAT', dzien: 25, active: true },
  { typ: 'PIT', dzien: 20, active: false },
];
...
export default function Dashboard({
  ...
  // Termin per-klient; dla klientów ze spóźnionymi dokumentami zasiewamy termin w przeszłości,
  // żeby „dni po terminie" były widoczne w demo (Fala 2, pkt 3). Wariacja 3–8 dni dla realizmu.
  const [deadlines, setDeadlines] = useState<Record<string, string>>(() => {
    const init: Record<string, string> = {};
    clients.forEach((c, idx) => {
      if (c.documents.some(d => d.status === 'Spóźnione')) {
        const d = new Date();
        d.setDate(d.getDate() - (3 + (idx % 6)));
        init[c.id] = d.toISOString().slice(0, 10);
      }
    });
    return init;
  });
  const [rejectModal, setRejectModal] = useState<{ key: string; clientId: string; docId: string; fileName: string } | null>(null);
  const [rejectReasonInput, setRejectReasonInput] = useState('');
  const [reminderTab, setReminderTab] = useState<'deadline' | 'reminders'>('deadline');
  const [reminders, setReminders] = useState<Record<string, Przypomnienie[]>>({});
  const getReminders = (id: string) => reminders[id] ?? DEFAULT_PRZYPOMNIENIA;
  const setClientReminders = (id: string, list: Przypomnienie[]) => setReminders(prev => ({ ...prev, [id]: list }));
  ...
  const requestReject = (clientId: string, docId: string, idx: number, fileName: string) => {
    ...
    if (rejectedFiles.has(key)) {
      ...
      toast('Cofnięto odrzucenie', { icon: '↩️', duration: 1500 });
    } else {
      setRejectReasonInput('');
      setRejectModal({ key, clientId, docId, fileName });
    }
  };

  const confirmReject = (reason: string) => {
    if (!rejectModal || !reason.trim()) return;
    const { key, clientId, docId, fileName } = rejectModal;
    setRejectedFiles(prev => { const n = new Set(prev); n.add(key); return n; });
    setApprovedFiles(prev => { const n = new Set(prev); n.delete(key); return n; });
    setDocRejection?.(clientId, docId, reason.trim());
    toast(`Odrzucono: ${fileName} — klient zobaczy powód w portalu`, { icon: '📩', duration: 2800 });
    setRejectModal(null);
  };

  // Dni po terminie z ustawionego terminu klienta (Fala 2, pkt 3)
  const daysLate = (clientId: string) => {
    const dl = deadlines[clientId];
    if (!dl) return 0;
    const diff = Math.floor((Date.now() - new Date(dl).getTime()) / 86400000);
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="font-bold text-slate-900 text-base lg:text-lg flex items-center gap-2 flex-wrap">
                          ...
                          {!client.locked && daysLate(client.id) > 0 && client.documents.some(d => d.status === 'Spóźnione') && (
                            <span className="flex items-center gap-1 px-2 py-0.5 bg-rose-100 text-rose-600 rounded-lg text-[10px] font-black uppercase tracking-wide">
                              <Clock className="w-3 h-3" /> {t('list.days_late', { count: daysLate(client.id) })}
                            </span>
                          )}
                        </div>
...
>
                          ...
                          <AnimatePresence>
                            {viewingFilesDocId === `${client.id}-${doc.id}` && (() => {
                              ...
                              >
                                ...
                                {doc.rejectionReason && (
                                  <div className="mb-2 px-2.5 py-2 bg-pink-50 border border-pink-100 rounded-xl text-[11px] text-pink-600 leading-snug">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
              <div className="p-8">
                ...
                {/* Termin + Przypomnienia podatkowe (zakładki — odwzorowane z produkcji) */}
                <div className="mb-5 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mb-4">
                    <button onClick={() => setReminderTab('deadline')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${reminderTab === 'deadline' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Termin dokumentów</button>
                    <button onClick={() => setReminderTab('reminders')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${reminderTab === 'reminders' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Przypomnienia podatkowe</button>
                  </div>

                  {reminderTab === 'deadline' && (
                    <div>
                  ...
                  {reminderTab === 'reminders' && (
                    <div>
                      <p className="text-xs text-slate-500 mb-3">Klient otrzyma email danego dnia miesiąca. Kopia trafia do biura.</p>
                      <div className="space-y-2">
                        {getReminders(editingClient.id).map((r, i) => (
                          <div key={i} className="flex items-center gap-2 p-2.5 bg-white rounded-xl border border-slate-100">
                            <input type="checkbox" checked={r.active} onChange={e => setClientReminders(editingClient.id, getReminders(editingClient.id).map((x, xi) => xi === i ? { ...x, active: e.target.checked } : x))} className="rounded" />
                            {r.typ === 'custom' ? (
                              <input type="text" value={r.label ?? ''} placeholder="np. PCC-3, akcyza…" onChange={e => setClientReminders(editingClient.id, getReminders(editingClient.id).map((x, xi) => xi === i ? { ...x, label: e.target.value } : x))} className="text-xs font-semibold text-slate-700 flex-1 bg-transparent border-b border-slate-200 focus:outline-none focus:border-blue-400" />
                            ) : (
                              <span className="text-xs font-semibold text-slate-700 flex-1">{TYP_PODATKOWE[r.typ]}</span>
                            )}
                            <div className="flex items-center gap-1">
                              <span className="text-[10px] text-slate-400">dnia</span>
                              <input type="number" min={1} max={28} value={r.dzien} onChange={e => setClientReminders(editingClient.id, getReminders(editingClient.id).map((x, xi) => xi === i ? { ...x, dzien: Math.min(28, Math.max(1, Number(e.target.value))) } : x))} className="w-12 text-xs px-2 py-1 border border-slate-200 rounded-lg text-center bg-white focus:outline-none focus:ring-1 focus:ring-blue-300" />
                            </div>
                            <button onClick={() => setClientReminders(editingClient.id, getReminders(editingClient.id).filter((_, xi) => xi !== i))} className="text-slate-300 hover:text-red-400 transition-colors"><X className="w-3.5 h-3.5" /></button>
                          </div>
                        ))}
                        <button onClick={() => setClientReminders(editingClient.id, [...getReminders(editingClient.id), { typ: 'custom', label: '', dzien: 15, active: true }])} className="w-full py-2 text-xs font-semibold text-blue-600 border border-dashed border-blue-300 rounded-xl hover:bg-blue-50 transition-colors flex items-center justify-center gap-1"><Plus className="w-3 h-3" /> Dodaj termin</button>
                        <button onClick={() => toast.success('Przypomnienia zapisane', { duration: 2000, style: { borderRadius: '12px' } })} className="w-full py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-colors">Zapisz przypomnienia</button>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {vClient.documents.map(doc => {
                    ...
                    return (
                      <div key={doc.id}>
                        ...
                        {doc.rejectionReason && (
                          <div className="mb-2 px-3 py-2 bg-pink-50 border border-pink-100 rounded-xl text-[11px] text-pink-600 leading-snug">
                            <b className="uppercase tracking-wide">{t('reject.badge')}:</b> {doc.rejectionReason}
                          </div>
                        )}
                        {allFiles.length === 0 ? (
  ...
  {showWelcome && (
    ...
    {/* MODAL POWODU ODRZUCENIA (Fala 2 — pkt 9) */}
    <AnimatePresence>
      {rejectModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setRejectModal(null)} className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" />
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md bg-white rounded-[2rem] shadow-2xl p-6">
            <div className="flex items-start gap-3 mb-1">
              <div className="p-2 bg-pink-50 text-pink-500 rounded-xl shrink-0"><Info className="w-5 h-5" /></div>
              <div>
                <h3 className="text-lg font-black text-slate-900">{t('reject.title')}</h3>
                <p className="text-xs text-slate-500 mt-0.5 truncate max-w-[280px]">{rejectModal.fileName}</p>
              </div>
            </div>
            <p className="text-[11px] text-slate-400 mb-4">{t('reject.subtitle')}</p>
            <div className="flex flex-wrap gap-2 mb-3">
              {['r1', 'r2', 'r3', 'r4'].map(rk => (
                <button key={rk} onClick={() => setRejectReasonInput(t(`reject.${rk}`))}
                  className={`px-3 py-1.5 rounded-xl text-[11px] font-bold border transition-all ${rejectReasonInput === t(`reject.${rk}`) ? 'bg-pink-500 text-white border-pink-500' : 'bg-white text-slate-500 border-slate-200 hover:border-pink-300'}`}>
                  {t(`reject.${rk}`)}
                </button>
              ))}
            </div>
            <textarea value={rejectReasonInput} onChange={e => setRejectReasonInput(e.target.value)} rows={3}
              placeholder={t('reject.placeholder')}
              className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-400/40 focus:border-pink-400 transition-all mb-4" />
            <div className="flex gap-2">
              <button onClick={() => setRejectModal(null)} className="flex-1 py-3 rounded-2xl bg-slate-100 text-slate-500 font-bold text-sm hover:bg-slate-200 transition-all">{t('reject.cancel')}</button>
              <button onClick={() => confirmReject(rejectReasonInput)} disabled={!rejectReasonInput.trim()}
                className="flex-1 py-3 rounded-2xl bg-pink-500 text-white font-bold text-sm hover:bg-pink-600 disabled:opacity-50 transition-all">{t('reject.confirm')}</button>
            </div>
          </motion.div>
        </div>
      )}
